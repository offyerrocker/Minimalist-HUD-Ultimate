local addon_id = "template"
return addon_id,{
	name = "Template Addon 1",	--this is the name shown to the user in the in-game menu
	desc = "Hello there!",		--this is the description shown when mousing-over the addon in the in-game menu
	autodetect_assets = true,	--only useful if you have custom assets to load (like textures or fonts).
								--if you are using existing assets already loaded by PAYDAY 2 or another mod, you do not need this.
	layer = 1,					--the render order of this panel. higher numbers are on top
	categories = {
								--optional
	...
	create_func = function(addon,parent_panel)
		...
		--addon:
			...
			--without cluttering the global table or the internal addon data table that MHUDU uses to organize addons.
		...
		local example_bitmap = parent_panel:bitmap({
			name = "hello",
		...
		addon.example_bitmap = example_bitmap
		...
		MHUDU:AddListener("set_criminal_health","testaddon_oncriminalhealthset",{
			callback = function(id,data)
				if id == HUDManager.PLAYER_PANEL then 
					addon.example_bitmap:set_color(Color(1,math.random(),1))
